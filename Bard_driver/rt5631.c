...
static int rt5631_reg_init(struct snd_soc_codec *codec) //init the codec reg.
{
	int i;
...
static int rt5631_volatile_register(struct snd_soc_codec *codec, //judge if the reg. is unchanged
				    unsigned int reg)
{
	switch (reg) {
	case RT5631_RESET:
...
static int rt5631_readable_register(struct snd_soc_codec *codec, //reag only.
				    unsigned int reg)
{
	switch (reg) {
	case RT5631_RESET:
...
static const struct pll_div codec_master_pll_div[] = {
	...
	{16934400,  11289600,  0x0621},
	{24000000,  11289600, 0x0421 },
	...
	struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	printk("--rt5631_hifi_codec_set_dai_fmt---");
	dev_dbg(codec->dev, "enter %s\n", __func__);
				...
				int clk_id, unsigned int freq, int dir)
{
	...
	printk("--rt5631_hifi_codec_set_dai_sysclk----\n");
		...
		int source, unsigned int freq_in, unsigned int freq_out)
{
	...
	printk("--rt5631_codec_set_dai_pll-start-\n");
	if (!freq_in || !freq_out) {
	...
	printk("--rt5631_codec_set_dai_pll-end-\n");
	return ret;
}
...
static int rt5631_probe(struct snd_soc_codec *codec)
{
	...
	if (val & 0x0002)
		rt5631->codec_version = 1; //find the codec
	else
...
MODULE_AUTHOR("Alex <alex_tang@realsil.com.cn>");
MODULE_LICENSE("GPL");
